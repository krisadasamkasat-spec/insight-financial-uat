...
const TransactionStatusModal = ({ isOpen, onClose, onConfirm, data, currentBalance }) => {
    ...
    if (type === 'expense') {
        // Paying uses money -> Deduct
        if ((newStatus === 'Paid' || newStatus === 'จ่ายแล้ว') && (oldStatus !== 'Paid' && oldStatus !== 'จ่ายแล้ว')) {
            impact = 'deduct';
        }
        // Reverting paid expense -> Add money back
        else if ((oldStatus === 'Paid' || oldStatus === 'จ่ายแล้ว') && (newStatus !== 'Paid' && newStatus !== 'จ่ายแล้ว')) {
            impact = 'add';
        }
    } else if (type === 'income') {
        // Receiving income -> Add money
        if ((newStatus === 'Received' || newStatus === 'received' || newStatus === 'ได้รับแล้ว') && (oldStatus !== 'Received' && oldStatus !== 'received' && oldStatus !== 'ได้รับแล้ว')) {
            impact = 'add';
        }
        // Reverting received income -> Deduct money
        else if ((oldStatus === 'Received' || oldStatus === 'received' || oldStatus === 'ได้รับแล้ว') && (newStatus !== 'Received' && newStatus !== 'received' && newStatus !== 'ได้รับแล้ว')) {
            impact = 'deduct';
        }
    }
